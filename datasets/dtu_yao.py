...
class MVSDataset(Dataset):
    ...
    def build_list(self): # 构建训练样本条目，最终的meta数组中共用27097条数据
        metas = []
        ...
        # scans
        for scan in scans: # 训练集共有79个场景
            pair_file = "Cameras/pair.txt"
            # read the pair file
            with open(os.path.join(self.datapath, pair_file)) as f:
                num_viewpoint = int(f.readline()) # 每个场景的图片个数
                # viewpoints (49)
        ...
        print("dataset", self.mode, "metas:", len(metas)) # metas:27097长度,79个场景，每个场景49张图，每张图有10张源视角？（可能并不是都有10张）
        return metas
    ...
    def __getitem__(self, idx):
        ...
        view_ids = [ref_view] + src_views[:self.nviews - 1] # 使用另外两张图一起做深度估计
        ...
        for i, vid in enumerate(view_ids):
            ...
            proj_mat[:3, :4] = np.matmul(intrinsics, proj_mat[:3, :4]) # 注意这里已经乘以了K矩阵
            proj_matrices.append(proj_mat)

            if i == 0:  # 如果是参考视角，获得采样的深度值，mask，和GT深度
                depth_values = np.arange(depth_min, depth_interval * self.ndepths + depth_min, depth_interval,
                                         dtype=np.float32) # ndepths：是指的不同的深度数量，最后获得192个等间隔的深度
