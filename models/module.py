...
def homo_warping(src_fea, src_proj, ref_proj, depth_values):
    ...
    with torch.no_grad(): # 这种算法很奇怪
        proj = torch.matmul(src_proj, torch.inverse(ref_proj))  # 首先获取到从参考视角到源视角的投影矩阵K【R|T】
        rot = proj[:, :3, :3]  # [B,3,3]
        ...
        xyz = torch.stack((x, y, torch.ones_like(x)))  # [3, H*W],创建了参考视角归一化像平面的齐次坐标[u,v,1]
        xyz = torch.unsqueeze(xyz, 0).repeat(batch, 1, 1)  # [B, 3, H*W]
        rot_xyz = torch.matmul(rot, xyz)  # [B, 3, H*W] 类似得到源视角坐标系下的坐标（但是还未加上T）
        rot_depth_xyz = rot_xyz.unsqueeze(2).repeat(1, 1, num_depth, 1) * depth_values.view(batch, 1, num_depth,
                                                                                            1)  # 得到每个深度下，源视角相机坐标系下的坐标（还未加上T）[B, 3, Ndepth, H*W]
        proj_xyz = rot_depth_xyz + trans.view(batch, 3, 1, 1)  # 得到每个深度源视角相机坐标系下的坐标 [B, 3, Ndepth, H*W]
        proj_xy = proj_xyz[:, :2, :, :] / proj_xyz[:, 2:3, :, :]  # [B, 2, Ndepth, H*W]得到源视角归一化像平面坐标？
        proj_x_normalized = proj_xy[:, 0, :, :] / ((width - 1) / 2) - 1 # 归一化到-1-1之间
        proj_y_normalized = proj_xy[:, 1, :, :] / ((height - 1) / 2) - 1
        proj_xy = torch.stack((proj_x_normalized, proj_y_normalized), dim=3)  # [B, Ndepth, H*W, 2]
        grid = proj_xy

    warped_src_fea = F.grid_sample(src_fea, grid.view(batch, num_depth * height, width, 2), mode='bilinear',
                                   padding_mode='zeros') # # 按照grid中的映射关系，取src特征图的元素
    warped_src_fea = warped_src_fea.view(batch, channels, num_depth, height, width) # reshape
...
def depth_regression(p, depth_values):
    ...
    depth = torch.sum(p * depth_values, 1) # 加权求和
    return depth
